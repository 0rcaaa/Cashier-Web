...
<body
                ...
                <div class="flex flex-wrap justify-between items-end gap-4 mb-4">
                  ...
                  <div class="flex items-end gap-2">
                    <div class="flex flex-col">
                      <label for="filter_status">Search Member</label>
                      <input type="text" id="search" placeholder="Search by Name or Phone" class="border rounded p-2" />
                    </div>
  ...
  <script>
    ...
    //fetch data from api
      ...
      return fetch(`<?= base_url()?>/service/api.php?action=getMembers&${query}`)
        .then(response =>{
          if (!response.ok){
      ...
      data.forEach(item => {
        TB.insertAdjacentHTML('beforeend', `
              <tr>
                  ...
                  <td class="text-center px-4 py-2">${item.Phone}</td>
                  <td class="text-center px-4 py-2">${item.Points}</td>
                  ...
                  <td class="text-center px-4 py-2"><button onclick="document.location='edit_member.php?member=${item.id}'" class="bg-primary text-white px-3 py-1 rounded">Edit</button>
                  <button onclick="if(confirm('Yakin?')) location.href='<?=base_url()?>/service/delete.php?type=members&id=${item.id}'" class="bg-red-500 text-white px-3 py-1 rounded">Delete</button>
                  </td>
              </tr>
          `);
