...
<body
            ...
            <div class="max-w-full overflow-x-auto">
              ...
              <div class="flex flex-wrap justify-between gap-4 mb-4">
                ...
                <div class="flex flex-wrap gap-4 mb-4">
                  <select id="filter_member" class="border rounded p-2">
                    <option value="">All Members</option>
                    <option value="registered">Registered</option>
                    <option value="unregistered">Unregistered</option>
                  </select>

                  <select id="filter_status" class="border rounded p-2">
                  ...
                  <input type="date" id="filter_date" class="border rounded p-2" />
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      document.getElementById('filter_member').addEventListener('change', () => applyFilter());
      document.getElementById('filter_status').addEventListener('change', () => applyFilter());
      document.getElementById('filter_date').addEventListener('change', () => applyFilter());

      // Event listener search (Enter key + button)
    ...
    function searchOrder() {
      ...
      if (search) {
        ...
        applyFilter(); // fallback load normal data
      }
    }
    ...
    function renderPagination(totalPages, currentPage = 1) {
      ...
      for (let i = 1; i <= totalPages; i++) {
        container.innerHTML += `
      <button onclick="applyFilter(${i})" class="px-3 py-1 rounded ${i === currentPage ? 'bg-primary text-white' : 'bg-gray-200'}">${i}</button>`;
      }
    }
  </script>
</body>
