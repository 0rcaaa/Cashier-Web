        ...
        <main>
          <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
            ...
            <div
              ...
              <!-- ====== Chart One Start -->
              <div
                class="col-span-12 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-6.5 ">
                <div class="flex flex-wrap items-start justify-between gap-3 sm:flex-nowrap">
                  <div class="flex w-full flex-wrap gap-3 sm:gap-5">
                    <div class="flex min-w-[13rem]">
                      <span
                  ...
                  <div class="flex w-full max-w-45 justify-end">
                    <div class="inline-flex items-center rounded-md bg-whiter py-3 px-5 gap-5 dark:bg-meta-4">
                      <div class="flex flex-col ">
                        <label for="filter_date_from">From</label>
                        <input type="date" name="date(from)" id="filter_date_from"
                          class="border rounded p-2 w-30 rounded bg-white text-xs font-medium text-black shadow-card hover:bg-white hover:shadow-card dark:bg-boxdark dark:text-white dark:hover:bg-boxdark " />
                      </div>

                      <div class="flex flex-col">
                        <label for="filter_date_to">To</label>
                        <input type="date" name="date(to)" id="filter_date_to"
                          class="border rounded p-2 w-30 rounded bg-white text-xs font-medium text-black shadow-card hover:bg-white hover:shadow-card dark:bg-boxdark dark:text-white dark:hover:bg-boxdark" />
                      </div>
                    </div>
                  </div>
                ...
                <div id="chartSatuContainer">
                  <canvas id="chartSatu" class="-ml-5 dark:text-black"></canvas>
                </div>
              </div>
    ...
    <script>
      document.addEventListener("DOMContentLoaded", function () {
        ...
        async function fetchChartData(from, to) {
          const res = await fetch("./data_chart.php", {
            method: "POST",
            headers: {
            ...
            const keuntungan = response.data?.keuntungan || [];
            const penjualan = response.data?.penjualan || [];

            const chartSatuElem = document.getElementById("chartSatu");
            if (!chartSatuElem) return;

            const ctx = chartSatuElem.getContext("2d");
          ...
          const dateRange2Elem = document.getElementById("dateRange2");
          if (dateRangeElem)
           {
            dateRangeElem.textContent = `${from} - ${to}`
            dateRangeElem.classList.add("block");
           };
          if (dateRange2Elem) dateRange2Elem.textContent = `${from} - ${to}`;
        }

        // Event listener input tanggal
        const filterFrom = document.getElementById("filter_date_from");
        const filterTo = document.getElementById("filter_date_to");

        if (filterFrom && filterTo) {
          filterFrom.addEventListener("change", () => {
            const from = filterFrom.value;
            const to = filterTo.value;
            if (from && to) fetchChartData(from, to);
          });
          ...
          filterFrom.value = sevenDaysAgo;
          filterTo.value = today;
          fetchChartData(sevenDaysAgo, today);
        }
      });
    </script>
    </body>
