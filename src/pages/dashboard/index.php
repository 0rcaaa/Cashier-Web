...
<body
      ...
      <main>
        <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
          ...
          <div
            ...
            <!-- ====== Chart One Start -->
            <div
              class="col-span-12 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-6.5 xl:col-span-7">
              <div class="flex flex-wrap items-start justify-between gap-3 sm:flex-nowrap">
                <div class="flex w-full flex-wrap gap-3 sm:gap-5">
                  <div class="flex min-w-47.5">
                    ...
                    <div class="w-full">
                      <p class="font-semibold text-primary">Total Keuntungan</p>
                      <p class="text-sm font-medium" id="dateRange">Loading...</p>
                    </div>
                  </div>
                  <div class="flex min-w-47.5">
                    <span
                    ...
                    <div class="w-full">
                      <p class="font-semibold text-secondary">Total Penjualan</p>
                      <p class="text-sm font-medium" id="dateRange2">Loading...</p>
                    </div>
                  </div>
                </div>
              ...
              <div id="chartSatuContainer">
                <div id="chartSatu" class="-ml-5 dark:text-black"></div>
              </div>
            </div>
            ...
            <div
              ...
              <div class="mb-2">
                <div
                  id="chartThree"
                  class="mx-auto flex justify-center"></div>
              </div>
              <div
  ...
  <!-- <script>
    ...
    .then(response => response.json())
        .then(data => {
            let labels = data.map(row => row.bulan);
            let keuntungan = data.map(row => row.keuntungan);
            let total_penjualan = data.map(row => row.total_penjualan);

            const ctx = document.getElementById('chart_satu').getContext('2d');
            new Chart(ctx, {
                type: 'line',
                data: {
                    labels: labels,
                    datasets: [
                        ...
                        },
                        {
                            ...
                            borderColor: '#f43f5e', // Warna merah
...
<script>
  ...
  <script src="https://cdn.jsdelivr.net/npm/apexcharts">
    ...
    async function chart01() {
      try {
        const response = await fetch("data_chart.php"); // Ambil data dari backend
        const jsonData = await response.json();

        const chartContainer = document.querySelector("#chartSatuContainer"); // Container utama
        const chartSelector = document.querySelector("#chartSatu"); // Elemen chart

        // Jika data kosong, tampilkan pesan
        if (!Array.isArray(jsonData) || jsonData.length === 0) {
          chartContainer.innerHTML = "<p class='text-center text-gray-500'>Tidak ada data</p>";
          return;
        }

        // Set tanggal range berdasarkan data pertama dan terakhir
        document.querySelector("#dateRange").innerText = jsonData[0].bulan + " - " + jsonData[jsonData.length - 1].bulan;
        document.querySelector("#dateRange2").innerText = jsonData[0].bulan + " - " + jsonData[jsonData.length - 1].bulan;

        // Ekstrak data untuk chart
        const categories = jsonData.map(item => item.bulan);
        const totalPenjualan = jsonData.map(item => item.total_penjualan);
        const totalKeuntungan = jsonData.map(item => item.keuntungan);

        // Konfigurasi chart dengan data dari backend
        const chartSatuOptions = {
          series: [{
        ...
        if (chartSelector) {
          const chartSatu = new ApexCharts(chartSelector, chartSatuOptions);
          chartSatu.render();
        }
